...
def suma_vectores(vec1, vec2):
    ...
    if (len(vec1) == len(vec2)):
        ...
        return vec_suma
print(suma_vectores([4+5j,2+3j],[2+3j,1+2j]))
    #else:
...
def adjoint(mat):
    mat_transpose = mat_transpuesta(mat)
...
def check_unit(mat):
    if len(mat) == len(mat[0]):
        identity = [[0 for rep in range(len(mat))]for rep in range(len(mat))]
        for i in range(len(mat)):
            for j in range(len(mat)):
                if i == j:
                    identity[i][j] = 1
        ...
        checker = False
        adj = adjoint(mat)
        mat_mult = multiplicacion(mat, adj)
        for i in range(len(mat)):
            for j in range(len(mat)):
                if mat[i][j] == mat_mult[i][j]:
                    checker = True
        return checker
    else:
